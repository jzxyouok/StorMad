...
$(function() {
	...
	var fillTable=function(page) {
		...
		for(var i = 0; i < list.length; i++) {
			...
			if(list[i].status == 0) {
				tbl += '<td><a type="button" class="btn btn-default" href="javascript:bindAd(\''+ list[i].bpaId +'\', \''+ list[i].bpId +'\', \''+ list[i].location +'\')">立即绑定</a></td>';
			} else {
...
//绑定广告
var bindAd=function(bpaId, bpId, local) {
	location.href = encodeURI('ad-binding.html?bpaId='+ bpaId +'&bpId='+bpId+'&local='+local, "UTF-8");
