...
Bidplan.prototype = {
        ...
        api.post('/api/bid/bid.jsp', {
            ...
            if (0 === res.errCode && true === res.obj) {
                ...
                layer.open({
                    ...
                    shadeClose: true,
                    content: '<div id="payTips" class="frame-content"><div class="frame-body pay-tips-02"><div class="top"><p class="warning">恭喜您出价成功╰(￣▽￣)╮</p><p>您的出价：<span>￥' + $('#currentBidMoney').val() + '</span></p></div><div class="bottom"><button type="button" class="btn btn-default" id="btnBidSuccess">查看竞拍结果</button></div></div></div>',
                    end: function() {
    ...
    descDisabled: function() {
        $('#currentBidMoney').val() <= this.lowestMoney ? $('.desc').addClass('disabled') : $('.desc').removeClass('disabled');
    },
            ...
            if (0 === res.errCode) {
                ...
                $.each(res.obj.record, function(n, item) {
                    resultList += api.simple(resultItem, {
                        ...
                        bpName: item.bpName,
                        time: self.formatDate(item.cstartDate) + '至' + self.formatDate(item.cendDate),
                        iname: item.iname,
    ...
    bindEvent: function() {
        ...
        $('#check_list').on('click', '.store-List', function() {
            ...
            if ($(this).is(':checked')) {
                ...
                // 倒计时
                if (self.bidTime.bidStartTime - (new Date()).getTime() < 0) {
                    $('.endCountDown').show();
                    time = setInterval(function() {
                        $('#countdown').html(self.countdown(self.bidTime.bidEndTime));
                    }, 1000);
                } else {
                    $('.endCountDown').hide();
                }
