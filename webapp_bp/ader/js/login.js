...
$(function() {
    var getVCode = function(vid) {
        ...
        $.ajax({
            type: 'GET',
            url: yonghui.contextPath + '/api/createCode.jsp?timestamp=' + new Date().getTime(),
            data: {},
            dataType: 'json',
    ...
    $('#btnSubmit').click(function() {
        ...
        if (loginName == '') {
            $('.error-tips').html('<i class="exclamation-icon"></i>请输入用户名').addClass('shake');
            setTimeout(function() {
                $('.error-tips').removeClass('shake')
            }, 500);
            // alert('请输入用户名');
            $('#loginName').focus();
            getVCode();
        ...
        if (password == '') {
            $('.error-tips').html('<i class="exclamation-icon"></i>请输入密码').addClass('shake');
            setTimeout(function() {
                $('.error-tips').removeClass('shake')
            }, 500);
            // alert('请输入密码');
        ...
        var vcode = $('#vcode').val();
        if (vcode == '') {
            $('.error-tips').html('<i class="exclamation-icon"></i>请输入验证码').addClass('shake');
            setTimeout(function() {
                $('.error-tips').removeClass('shake')
            }, 500);
            // alert('请输入验证码');
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.errCode == 0) {
                    ...
                    $('.error-tips').html('<i class="exclamation-icon"></i>' + data.errMsg + '').addClass('shake');
                    setTimeout(function() {
                        $('.error-tips').removeClass('shake')
                    }, 500);
                    getVCode();
