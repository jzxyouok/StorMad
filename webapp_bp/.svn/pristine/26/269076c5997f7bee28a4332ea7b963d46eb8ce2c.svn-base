...
;
        //初始化分页参数
        $.post(yonghui.contextPath + "/api/log/findBpOpLogPage.jsp", req, function(data) {
    ...
    layui.use('laydate', function() {
        ...
        if (val.length > 0) {
            ...
            for (var i = 0; i < val.length; i++) {
                ...
                htm += '<td class="crt-time">' + myOpTime + '</td><td>' + val[i].content + '</td><td>' + val[i].adName + '</td>';
...
function query(pageNo, opTime, content) {
    ...
    var req = { pageNo: pageNo, pageSize: yonghui.pageSize, opTime: opTime, content: content };
    $.post(yonghui.contextPath + "/api/log/findBpOpLogPage.jsp", req, function(data) {
        if (data.errCode == 0) {
            get_info(data);
        } else {
