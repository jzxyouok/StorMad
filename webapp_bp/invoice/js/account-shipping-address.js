        ...
        api.post('/api/invoice/queryAddr.jsp', {
            ...
            if (0 === res.errCode) {
                if (res.obj.record && res.obj.record.length) {
                    ...
                    $('.no_info').hide();
                    $('.show_info').show();
                    $('.apply-Invoice').css({'color':'#fff','background-color':'#f5715f','border':'1px solid #c74545'}).removeAttr('disabled');
                } else {
                    $('.no_info').show();
                    $('.show_info').hide();
                    $('.apply-Invoice').css({'color':'#fff','background-color':'#ccc','border':'1px solid #ccc'}).attr('disabled','disabled');
                    // $('.tab-addr').hide();
                }
            }
        });
