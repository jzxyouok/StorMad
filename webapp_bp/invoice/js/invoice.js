    ...
    layui.use('laydate', function() {
        ...
        if (val.length > 0) {
            for (var i = 0; i < val.length; i++) {
                ...
                htm += '<tr data-ivId="' + val[i].ivId + '">';

                htm += '<td>' + laydate.now(val[i].applyTime, "YYYY-MM-DD hh:mm:ss") + '</td>';

                htm += '<td>' + titlename + '</td><td>' + val[i].invoiceNo + '</td><td>￥' + ((val[i].money) / 100).toFixed(2) + '</td>';
...
function query(pageNo, yearMonth, money, status) {
    ...
    var curmoney = $('.btn-money').val();

    var req = { pageNo: pageNo, pageSize: yonghui.pageSize, yearMonth: curyearMonth, money: curmoney, status: curstatus };
