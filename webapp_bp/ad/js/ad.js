...
NewAd.prototype = {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.errCode == 0) {
                    ...
                    var list = data.obj.record;
                    if (data.obj.record.length > 0) {
                        var options = '<select name="groups" id="groups"><option value="">选择推广组</option>';
                        for (var i = 0; i < list.length; i++) {
                            options += '<option value=\'' + list[i].sgId + '\'>' + list[i].sgName + '</option>';
                        }
                    } else {
                        var options = '<select name="groups" id="groups" disabled><option value="">选择推广组</option>';
                        layer.alert('请先创建推广组');
                    }
                    options += '</select>'
                    // $('#groups').html(options);
                    $('.groups-wrap').html(options);
                    if (sgId) {
    ...
    fillAdSizes: function(adType, asId) {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.errCode == 0) {
                    data.obj[0].textMaxLength && $('#content').attr({ 'placeholder': '最多输入' + data.obj[0].textMaxLength + '个文字', 'maxlength': data.obj[0].textMaxLength });
                    var list = data.obj;
                    ...
                    $('#adSizes option:nth-child(2)').attr('selected', true);
                    if (asId) {
    ...
    submitAd: function() {
        ...
        if ((link.indexOf('http://') == -1) && (typeId != 2)) {
            layer.alert('链接地址不能为空，请使用http://开头的链接');
            return;
        }
            ...
            layui.upload({
                ...
                success: function(data) { //上传成功后的回调
                    if (data.errCode == 0) {
                        ...
                        $('#errMsg').html('<i class="exclamation-icon"></i>'+data.errMsg)
                    }
                },
                error: function(data) {
                    $('#errMsg').css('display', 'block');
...
$(function() {
    /*动态写入推广组下拉框*/
    $('.groups-wrap').html('<select name="groups" id="groups" disabled><option value="">选择推广组</option></select>');
    if (getQueryString('adId')) {
        new NewAd(getQueryString('adId'));
    } else {
        new NewAd();
    }
    if(getQueryString('adId')!=null){
        $('#adSizes').removeAttr('disabled');
    }
})
