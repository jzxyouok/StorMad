...
class Admin_log extends MY_Controller {
    ...
    public function ad_log($user=0, $content=0, $start_time=0, $end_time=0, $cur_page=1)
	{
	    $user = urldecode($user);
	    $content = urldecode($content);
	    $start_time = urldecode($start_time);
	    $end_time = urldecode($end_time);
	    $this->layout_data['title'] = "管理员操作日志";
	    ...
	    $config['base_url'] = base_url()."/admin_log/ad_log/{$user}/{$content}/{$start_time}/{$end_time}/";
	    $config['total_rows'] = $this->admin_model->get_ad_log_count($conditions);
	    ...
	    $data['ad_log'] = $this->admin_model->get_ad_log($conditions);
	    
	    $data['url'] = base_url()."admin_log/ad_log/{$user}/{$content}/{$start_time}/{$end_time}/";
	    $data['total_page'] = ceil($config['total_rows']/$config['per_page']);
	    $data['cur_page'] = $cur_page;
	    
		$this->render('admin_log/ad_log', $data);
	}
}
