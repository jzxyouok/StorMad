...
function get_info(data) {
    ...
    if (val.length > 0) {
        for (var i = 0; i < val.length; i++) {
            ...
            var titleArr = val[i].title.split(",");
            var titlecode = titleArr[0];
            var titlename = titleArr[1];

            htm += '<td>' + val[i].ivId + '</td><td>' + applyTime + '</td><td>' + val[i].corporation + '</td><td>' + titlename + '</td><td>' + (val[i].money / 100).toFixed(2) + '</td>';
            ...
            htm += '<td><a class="' + actcss + '" href="javascript:void(0)" ' + actclick + ' id="act_' + val[i].ivId + '" >' + statusname + '</a></td>';
...
function query(pageNo, corpName, yearMonth, status) {
    var curyearMonth = $('.btn-yearMonth').val();
    var curcorpName = $('.btn-corpName').val();
    var curstatus = $('.btn-status .layui-this').attr('lay-value');

    var redata = { pageNo: pageNo, pageSize: yonghui.pageSize, corpName: curcorpName, yearMonth: curyearMonth, status: curstatus };
...
function exportData() {
    $('.btn-export').click(function() {
        ...
        var curcorpName = $('.btn-corpName').val();
        var curstatus = $('.btn-status .layui-this').attr('lay-value');
        window.location.href = yonghui.contextPath + '/api/invoice/exportInvoice.jsp?yearMonth=' + curyearMonth + '&corpName=' + curcorpName + '&status=' + curstatus;
        layer.msg('已导出待开票数据');
...
function importData() {
    $('.btn-import').click(function() {
        layui.use('upload', function() {
            layui.upload({
                url: yonghui.contextPath + '/api/invoice/importInvoice.jsp', //上传接口
                ext: 'xls|xlsx',
